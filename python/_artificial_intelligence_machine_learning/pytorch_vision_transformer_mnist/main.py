...
if __name__ == '__main__':
	...
	if os.path.exists('./model.pth'):
		...
		_, axes = plt.subplots(ncols=5, figsize=(7, 2))
		plt.subplots_adjust(left=0.03, right=0.97, top=0.75, wspace=0.15)
		for idx, ax in enumerate(axes):
			img_grid = make_grid(img_patches[idx], nrow=IMG_SIZE // PATCH_SIZE, normalize=True, pad_value=0.5)
			img_grid = img_grid.permute(1, 2, 0)  # (C, H, W) -> (H, W, C)
			ax.imshow(img_grid)
			ax.axis('off')
		plt.suptitle('Images as sequences of patches', y=0.9)
