...
if __name__ == '__main__':
	...
	if os.path.exists('./model.h5'):
		...
		plot_image_grid(
			x_train[:32], rows=4, cols=8, padding=5, scale_factor=2,
	...
	plot_confusion_matrix(y_test.argmax(axis=1), preds, None, f'Test confusion matrix\n(F1 score: {f1:.3f})')
	...
	while drawing:
		...
		pred_lbl = font.render(f'{pred_vector.argmax()} ({pred_vector.max():.1%} sure)', True, 'green')
