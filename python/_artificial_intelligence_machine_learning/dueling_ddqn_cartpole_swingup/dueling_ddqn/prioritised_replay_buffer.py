...
from dueling_ddqn_cartpole_swingup.dueling_ddqn.constants import *


class SumTree:
	"""
	A SumTree is a binary tree in which the value of a parent node is the sum of its children.
	Leaf nodes store transition (state, action, reward, next_state, terminal) priorities, and
...
class PrioritisedReplayBuffer:
	def __init__(self):
		"""See dueling_ddqn/constants.py for descriptions"""
		...
		# Contains transitions of format: (state, action, reward, next_state, terminal)
		self.buffer = [None] * self.capacity
		self.write = 0
		self.size = 0

	def store_transition(self, state, action, reward, next_state, terminal):
		self.buffer[self.write] = (state, action, reward, next_state, terminal)
