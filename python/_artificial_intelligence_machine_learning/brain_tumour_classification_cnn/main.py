...
def create_data_loaders(df):
	...
	x = [
		...
		ProgressBar(df['img_path'], desc='Preprocessing images', unit='imgs')
	]
	x = torch.stack(x)
...
if __name__ == '__main__':
		...
		for epoch in range(1, NUM_EPOCHS + 1):
			...
			with torch.inference_mode():
				for x_val, y_val in val_loader:
					...
					val_loss_total += loss_func(logits, y_val).item()
					all_y_labels.append(y_val)
					all_y_preds.append(logits.argmax(dim=1))
	...
	with torch.inference_mode():
		for x_test, y_test in test_loader:
			...
			loss_total += loss_func(logits, y_test).item()
