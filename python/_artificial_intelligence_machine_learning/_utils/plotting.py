...
def plot_image_array(
		images, rows, cols, gap, scale_factor=1, outer_padding=20,
		background_rgb=(128, 176, 240), title_rgb=(0, 0, 0),
	...
	for idx, img in enumerate(images):
		...
		if scale_factor != 1:
			...
			is_int = int(scale_factor) == scale_factor
			img = cv.resize(
				img, (new_w, new_h),
				interpolation=cv.INTER_NEAREST if scale_factor > 1 and is_int else cv.INTER_LINEAR
			)
