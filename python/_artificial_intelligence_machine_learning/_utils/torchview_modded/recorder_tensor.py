...
class Recorder:
	"""
	Context Manager that sets modules forward and torch
	creation ops to record them in computation graph
	"""
...
def module_forward_wrapper(sequential_module_names, model_graph: ComputationGraph) -> Callable[..., Any]:
	...
	def _module_forward_wrapper(mod: nn.Module, *args: Any, **kwargs: Any) -> Any:
		...
		name = sequential_module_names.pop(0) if isinstance(mod, nn.Sequential) else type(mod).__name__
		cur_node = ModuleNode(mod, cur_depth, input_nodes, name=name)
		...
		traverse_data_inplace(input_recorder, attach_node(attach_kwargs, tensor_to_node))
		...
		output_recorder: OrderedSet[RecorderTensor] = (reduce_data_info(out, collect_tensor, OrderedSet()))

		traverse_data_inplace(output_recorder, process_output_node(cur_node))
		traverse_data_inplace(input_recorder, pop_after_forward, recorded_output=output_recorder)

		# remove auxiliary tensor nodes from recorder_tensor
		output_nodes: NodeContainer[TensorNode] = (reduce_data_info(out, collect_tensor_node, NodeContainer()))
...
class RecorderTensor(torch.Tensor):
	...
	def __torch_function__(cls: Any, func: Callable[..., Any] | ScriptMethod, types: Any, args: Any = (), kwargs: Any = None) -> Any:
		...
		cur_node = FunctionNode(func, cur_depth, args_nodes, name=func_name)
...
def reduce_data_info(recorded_data: Any, action_fn: Callable[..., Any], collected: L, **kwargs: Any) -> L:
	"""
...
def traverse_data_inplace(recorded_data: Any, action_fn: Callable[..., Any], **kwargs: Any) -> None:
	"""
	...
	elif isinstance(recorded_data, Iterable) and not isinstance(recorded_data, (str, torch.Tensor)):
		for r_d in recorded_data:
			traverse_data_inplace(r_d, action_fn, **kwargs)
	...
	def _func(recorded_tensor: RecorderTensor) -> None:
		...
		if isinstance(kwargs['parents'], ModuleNode):
			assert getattr(recorded_tensor, 'tensor_nodes', None) is not None, \
				f"RecorderTensor to be attached to the Node {kwargs['parents']} must have tensor node"
		assert isinstance(kwargs['parents'], (FunctionNode, ModuleNode)), \
			f"Node {kwargs['parents']} to which to attach must be either FunctionNode or ModuleNode"
...
def pop_after_forward(r_in: RecorderTensor, recorded_output: OrderedSet[RecorderTensor]) -> None:
	"""
...
def collect_tensor_node_id_dict(recorded_data: RecorderTensor, collected: dict[RecorderTensor, TensorNode]) -> None:
	if getattr(recorded_data, 'tensor_nodes', None):
		collected[recorded_data] = recorded_data.tensor_nodes[-1].main_node


def collect_tensor(recorded_data: RecorderTensor, collected: OrderedSet[RecorderTensor]) -> None:
	collected.add(recorded_data)


def collect_shape(recorded_data: RecorderTensor, collected: list[tuple[int, ...]]) -> None:
...
def insert_empty_pass_node(recorded_tensor: RecorderTensor, out_node: TensorNode) -> None:
	"""
