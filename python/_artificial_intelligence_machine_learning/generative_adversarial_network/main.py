...
if __name__ == '__main__':
	...
	if Path('./gen_model.pth').exists():
		...
		early_stopping = EarlyStopping(target=gen_model, patience=10, mode='min')
		...
		for epoch in range(1, NUM_EPOCHS + 1):
			...
			for batch_idx, img_batch in enumerate(prog_bar, start=1):
				gen_model.train()
			...
			prog_bar.finish(f'mean_gen_loss={mean_gen_loss:.4f}')
