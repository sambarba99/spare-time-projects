...
if __name__ == '__main__':
	...
	if task_choice in 'BM':
		x_train, y_train, x_val, y_val, x_test, y_test, labels, _ = load_csv_classification_data(
			path, train_size=0.7, val_size=0.15, test_size=0.15, x_transform=StandardScaler(), output_as_tensor=True
		)
	else:
	...
	early_stopping = EarlyStopping(target=model, patience=10, mode='min')
	...
	for epoch in range(1, NUM_EPOCHS + 1):
		...
		if task_choice == 'B':
			# Forward pass
			logits = model(x_train).squeeze()  # Class logits
			probs = torch.sigmoid(logits)      # Class probs (use sigmoid when binary)
			preds = probs.round().detach()     # Class predictions

			# Calculate loss and F1
			loss = loss_func(logits, y_train)
			metric = f1_score(y_train, preds)
			...
			val_metric = f1_score(y_val, val_preds)

		elif task_choice == 'M':
			logits = model(x_train).squeeze()
			probs = torch.softmax(logits, dim=-1)  # Use softmax when multi-class
			preds = probs.argmax(dim=1).detach()

			loss = loss_func(logits, y_train)
			metric = f1_score(y_train, preds, average='weighted')
			...
			with torch.inference_mode():
				logits = model(x_val).squeeze()
			val_preds = logits.argmax(dim=1)  # Same as computing probs then rounding
			val_loss = loss_func(logits, y_val).item()
			val_metric = f1_score(y_val, val_preds, average='weighted')

		elif task_choice == 'R':
			preds = model(x_train).squeeze()

			loss = loss_func(preds, y_train)
			metric = mae_loss(preds, y_train)
			...
			val_loss = loss_func(val_preds, y_val).item()
			val_metric = mae_loss(val_preds, y_val)
	...
	with torch.inference_mode():
		if task_choice == 'B':
			...
			test_metric = mae_loss(preds, y_test)

			print('Loss:', loss.item())
			print('MAE:', test_metric.item())
