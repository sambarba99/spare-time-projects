...
def create_data_loaders(df):
	...
	x = [
		...
		ProgressBar(x_path, desc='Preprocessing images', unit='imgs')
	]
	...
	for img in ProgressBar(x_train, desc='Calculating mean and std of x_train', unit='imgs'):
...
if __name__ == '__main__':
	...
	for p in Path('C:/Users/sam/Desktop/projects/datasets/utkface').glob('*.jpg'):
		age, gender, race = p.name.split('_')[:3]
	...
	rand_indices = torch.randperm(df.shape[0])[:16]
	...
	if Path('./model.pth').exists():
		...
		early_stopping = EarlyStopping(target=model, patience=10, mode='max', track_best_weights=False)
		...
		for epoch in range(1, NUM_EPOCHS + 1):
			...
			for x_train, y_train_age, y_train_gender, y_train_race in prog_bar:
				...
				age_preds, gender_logits, race_logits = model(x_train)

				age_loss = loss_func_age(age_preds.squeeze(), y_train_age)
			...
			with torch.inference_mode():
				for x_val, y_val_age, y_val_gender, y_val_race in val_loader:
					...
					age_preds, gender_logits, race_logits = model(x_val)
					age_preds = age_preds.squeeze()
					gender_logits = gender_logits.squeeze()

					gender_probs = torch.sigmoid(gender_logits)
					gender_preds = gender_probs.round()
					race_preds = race_logits.argmax(dim=1)
					...
					age_val_loss_total += loss_func_age(age_preds, y_val_age).item()
					age_val_mae_total += mae_func_age(age_preds, y_val_age).item()
					gender_val_loss_total += loss_func_gender(gender_logits, y_val_gender).item()
					race_val_loss_total += loss_func_race(race_logits, y_val_race).item()
	...
	age_loss_total = age_mae_total = gender_loss_total = race_loss_total = 0
	...
	with torch.inference_mode():
		for test_idx, (x_test, y_test_age, y_test_gender, y_test_race) in enumerate(test_loader):
			...
			age_preds, gender_logits, race_logits = model(x_test)
			age_preds = age_preds.squeeze()
			gender_logits = gender_logits.squeeze()
			...
			all_gender_logits.append(gender_logits.cpu())
			all_race_preds.append(race_preds.cpu())
			all_race_labels.append(y_test_race.cpu())
			all_race_logits.append(race_logits.cpu())

			age_loss_total += loss_func_age(age_preds, y_test_age).item()
			age_mae_total += mae_func_age(age_preds, y_test_age).item()
			gender_loss_total += loss_func_gender(gender_logits, y_test_gender).item()
			race_loss_total += loss_func_race(race_logits, y_test_race).item()
			...
			for idx, ax in enumerate(axes.flatten()):
				...
				age_preds = round(age_preds[idx].item())
				gender_preds = int(gender_preds[idx].item())
				race_preds = race_preds[idx].item()
				gender_pred_label = DATASET_DICT['gender'][gender_preds]
				race_pred_label = DATASET_DICT['race'][race_preds]

				ax.imshow(pil_image_transform(img.cpu()))
				ax.axis('off')
				ax.set_title(
					f'Pred: {age_preds}, {gender_pred_label}, {race_pred_label}'
	...
	age_loss = age_loss_total / len(test_loader)
	age_mae = age_mae_total / len(test_loader)
	gender_loss = gender_loss_total / len(test_loader)
	gender_f1 = f1_score(torch.cat(all_gender_labels), torch.cat(all_gender_preds))
	race_loss = race_loss_total / len(test_loader)
	race_f1 = f1_score(torch.cat(all_race_labels), torch.cat(all_race_preds), average='weighted')

	print(f'Age test loss: {age_loss:.2f}')
	print(f'Age test MAE: {age_mae:.2f}')
	print(f'Gender test loss: {gender_loss:.4f}')
	print(f'Gender test F1: {gender_f1:.4f}')
	print(f'Race test loss: {race_loss:.4f}')
	print(f'Race test F1: {race_f1:.4f}')

	# ROC curve for gender output
	gender_probs = torch.sigmoid(torch.cat(all_gender_logits))
	plot_roc_curve(torch.cat(all_gender_labels), gender_probs, 'Test ROC curve for gender classification')
	...
	race_preds = torch.cat(all_race_logits).argmax(dim=1)
	plot_confusion_matrix(
