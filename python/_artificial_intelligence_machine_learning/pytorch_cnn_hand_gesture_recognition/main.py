"""
PyTorch CNN for hand gesture recognition (digits 0-5) and bounding box regression
...
DATASET_PATH = 'C:/Users/sam/Desktop/projects/datasets/hand_digit_gestures'
IMG_SIZE = 64
BATCH_SIZE = 32
LEARNING_RATE = 1e-4
NUM_EPOCHS = 50
...
if __name__ == '__main__':
	# Convert raw video data to separate images, then these images into labelled data

	# for vid_path in glob.iglob(f'{DATASET_PATH}/raw_videos/*.mp4'):
	# 	vid_path = vid_path.replace('\\', '/')
	# 	print(f'\nExtracting frames from {vid_path}...')
	# 	class_ = vid_path.split('/')[-1].removesuffix('.mp4')
	# 	frames = []
	...
	# 		output_path = f'{DATASET_PATH}/frames/{class_}_{idx}.png'
	# 		cv.imwrite(output_path, cv.rotate(img, cv.ROTATE_90_CLOCKWISE))

	for img_path in glob.iglob(f'{DATASET_PATH}/frames/*.png'):
		# Step 1: Read image
		img = cv.imread(img_path)
		img = cv.resize(img, (img.shape[1] // 4, img.shape[0] // 4))
		mask = np.zeros(img.shape[:2], np.uint8)  # Initial mask

		# Step 2: Define initial background and foreground models
		bgdModel = np.zeros((1, 65), np.float64)
		fgdModel = np.zeros((1, 65), np.float64)

		# Step 3: Define ROI (rectangle around foreground object)
		# Format: (x, y, width, height)
		rect = (200, 200, img.shape[1] - 200, img.shape[0] - 200)  # Rough guess

		# Step 4: Apply GrabCut
		cv.grabCut(img, mask, rect, bgdModel, fgdModel, 5, cv.GC_INIT_WITH_RECT)

		# Step 5: Post-process the mask
		# 0 and 2 are background, 1 and 3 are foreground
		mask2 = np.where((mask == 2) | (mask == 0), 0, 1).astype(np.uint8)
		foreground = img * mask2[:, :, np.newaxis]

		# Step 6: Show or save the result
		cv.imshow("Original", img)
		cv.imshow("Foreground Extracted", foreground)
		cv.waitKey(0)
