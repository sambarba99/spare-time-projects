...
class Knapsack:
	...
	def calc_fitness(self, all_items, NUM_ITEMS, KNAPSACK_CAPACITY):
		self.fitness = self.total_value(all_items, NUM_ITEMS) \
			if self.total_weight(all_items, NUM_ITEMS) <= KNAPSACK_CAPACITY else 0

	def total_value(self, all_items, NUM_ITEMS):
		return sum(all_items[i].value for i in range(NUM_ITEMS) if self.item_config[i])

	def total_weight(self, all_items, NUM_ITEMS):
