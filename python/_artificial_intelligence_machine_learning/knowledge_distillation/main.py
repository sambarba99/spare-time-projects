...
def create_data_loaders():
	...
	y_labels = [p.parent.name for p in img_paths]
...
def train_student_with_kd(teacher_model, student_model, save_path):
	...
	early_stopping = EarlyStopping(target=student_model, patience=20, mode='max')
	...
	for epoch in range(1, NUM_EPOCHS + 1):
		...
		for x_train, y_train in prog_bar:
			...
			loss = ALPHA * ce_loss + (1 - ALPHA) * kl_div_loss
		...
		x_val, y_val = next(iter(val_loader))
		with torch.inference_mode():
			logits = student_model(x_val.to(DEVICE)).cpu()
...
def test(model, plot_title):
	...
	for idx, ax in zip(test_indices, axes.flatten()):
		img, y_pred_class_idx, y_pred_prob, y_label = x_test[idx], preds[idx], y_test_probs[idx], y_labels[idx]
