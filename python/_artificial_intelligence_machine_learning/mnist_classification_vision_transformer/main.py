...
if __name__ == '__main__':
	...
	plot_confusion_matrix(y_test, preds, None, f'Test confusion matrix\n(F1 score: {f1:.3f})')
	...
	while drawing:
		...
		with torch.inference_mode():
			logits = model(model_input.unsqueeze(dim=0).to(DEVICE))
		...
		pred_lbl = font.render(f'{probs.argmax()} ({probs.max():.1%} sure)', True, 'green')
	...
	transformer_output = model.transformer[0].fc_block[0](transformer_input).squeeze(dim=0)
