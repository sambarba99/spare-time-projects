...
if __name__ == '__main__':
	...
	with torch.inference_mode():
		val_preds = model(x_val.unsqueeze(dim=1)).squeeze()

	val_rmse = root_mean_squared_error(y_val, val_preds)
	plot_val_progress(x_val, y_val, val_preds, val_rmse, 0)

	# Training loop
	for epoch in range(1, NUM_EPOCHS + 1):
		model.train()
		preds = model(x_train.unsqueeze(dim=1)).squeeze()
		...
		val_rmse = root_mean_squared_error(y_val, val_preds)
		plot_val_progress(x_val, y_val, val_preds, val_rmse, epoch)
