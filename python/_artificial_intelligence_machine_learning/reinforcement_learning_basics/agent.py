...
class Agent:
	def __init__(self, gamma=0.9, alpha=0.2, epsilon=0.1):
		"""
		Args:
			gamma (float): return discount factor
	...
	def __choose_action_epsilon_greedy(self, state):
		if np.random.random() < self.epsilon:
			...
			# Choose best action (exploitation), breaking ties randomly
		...
		for _ in range(num_training_epochs):
			...
			while not terminal:
				...
				self.q_table[state][action] += self.alpha * (
					reward + self.gamma * self.q_table[new_state][new_action] - self.q_table[state][action]
				)
