...
class Vertex:
	...
	def get_neighbours(self, maze, maze_generation):
		...
		is_wall = maze_generation

		neighbours = []
		rows, cols = len(maze), len(maze[0])

		if self.x - dist >= 0 and maze[self.y][self.x - dist].is_wall == is_wall:
			neighbours.append(maze[self.y][self.x - dist])
		if self.x + dist < cols and maze[self.y][self.x + dist].is_wall == is_wall:
			neighbours.append(maze[self.y][self.x + dist])
		if self.y - dist >= 0 and maze[self.y - dist][self.x].is_wall == is_wall:
			neighbours.append(maze[self.y - dist][self.x])
		if self.y + dist < rows and maze[self.y + dist][self.x].is_wall == is_wall:
			neighbours.append(maze[self.y + dist][self.x])
