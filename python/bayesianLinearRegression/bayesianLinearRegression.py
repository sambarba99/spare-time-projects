...
def plot_regression(approx_data, x_train, y_train, x_test, y_test, lam, lower_bounds=None, upper_bounds=None):
	...
	if not (lower_bounds is None or upper_bounds is None):
		plt.fill_between(x_test.flatten(), lower_bounds, upper_bounds, color="#0080ff", alpha=0.2, zorder=0, label="Error")
...
# Compute the logarithm of the marginal likelihood for a Bayesian linear regression model with basis matrix
# phi and hyperparameters alpha and sigma^2
...
data_generator = bayesianutility.DataGenerator(SIGMA ** 2)
...
rbf_generator = bayesianutility.RBFGenerator(centres=centres, radius=1)
...
for lam in lam_vals:
	...
	neg_log_evidence = np.append(neg_log_evidence, -compute_log_marginal(phi_train, y_train, lam / SIGMA ** 2, SIGMA ** 2))
