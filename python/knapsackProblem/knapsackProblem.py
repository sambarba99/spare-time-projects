...
POP_SIZE = 200
MUTATION_RATE = 0.2
ELITISM_RATE = 0.02 # Proportion of fittest individuals to avoid mutation
CROSSOVER_RATE = 0.8
TOURNAMENT_SIZE = 2
...
def initialisePopulation():
	...
	for i in range(POP_SIZE):
		itemConfig = [random.randint(0, 1) for i in range(NUM_ITEMS)]
...
def selection(population, allItems):
	parents = []

	for i in range(POP_SIZE):
		tournamentIndividuals = random.sample(population, TOURNAMENT_SIZE)
		parents.append(findFittest(allItems, *tournamentIndividuals))
...
def crossover(parents, allItems):
	...
	for i in range(POP_SIZE - 1):
		...
		offspring[i] = findFittest(allItems, off1, off2)

	return offspring

# Single bit-flip mutation
def mutation(offspring, allItems):
...
def findFittest(allItems, *population):
...
while True:
	...
	itemValues = [random.randint(50, 500) for i in range(NUM_ITEMS)]
	itemWeights = [random.randint(1, 15) for i in range(NUM_ITEMS)]
	...
	for i in range(GENERATIONS):
		...
		parents = selection(population, allItems)
		offspring = crossover(parents, allItems)
		mutants = mutation(offspring, allItems)
		population = deepcopy(mutants)

		bestPopKnapsack = findFittest(allItems, *population)
