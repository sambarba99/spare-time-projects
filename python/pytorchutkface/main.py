...
def data_generator(*, preprocessed_images, df, idx, device):
	...
	for batch in batches:
		...
		batch_x = torch.tensor(images, dtype=torch.float).to(device)
		batch_y = [torch.tensor(ages, dtype=torch.float).to(device),
			torch.tensor(genders, dtype=torch.float).to(device),
			torch.tensor(races, dtype=torch.float).to(device)]
...
def main():
	...
	if choice == 'T':
		...
		num_train_batches = int(np.ceil(len(train_idx) / BATCH_SIZE))
		num_val_batches = int(np.ceil(len(val_idx) / BATCH_SIZE))
		...
		for epoch in range(20):
			...
			for train_batch in tqdm(train_gen, desc=f'Epoch: {epoch} | batch', total=num_train_batches, ascii=True):
				x, y = train_batch
				y_pred = model(x)
			...
			with torch.inference_mode():
				for val_batch in val_gen:
					...
					mean_val_loss += (age_val_loss * 0.1 + gender_val_loss * 10 + race_val_loss * 4) / num_val_batches
					...
					mean_age_val_mae += age_val_mae.item() / num_val_batches
					mean_gender_val_f1 += gender_val_f1 / num_val_batches
					mean_race_val_f1 += race_val_f1 / num_val_batches
	...
	num_test_batches = int(np.ceil(len(test_idx) / BATCH_SIZE))
	...
	with torch.inference_mode():
		for test_batch in test_gen:
			...
			mean_age_test_loss += age_test_loss.item() / num_test_batches
			mean_gender_test_loss += gender_test_loss.item() / num_test_batches
			mean_race_test_loss += race_test_loss.item() / num_test_batches
			mean_age_test_mae += age_test_mae.item() / num_test_batches
			mean_gender_test_f1 += gender_test_f1 / num_test_batches
			mean_race_test_f1 += race_test_f1 / num_test_batches
