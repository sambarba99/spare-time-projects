...
class GridEnv:
		...
		for y in range(self.size):
			for x in range(self.size):
				...
				if self.__is_terminal((y, x)):
					...
					if best_action == NORTH: pg.draw.polygon(scene, ARROW_COL, self.__shift_arrow(NORTH_ARROW, x, y))
					if best_action == EAST: pg.draw.polygon(scene, ARROW_COL, self.__shift_arrow(EAST_ARROW, x, y))
	...
	def __map_range(self, x, from_lo, from_hi, to_lo, to_hi):
		if from_hi - from_lo == 0:
			return to_hi
		return (x - from_lo) / (from_hi - from_lo) * (to_hi - to_lo) + to_lo

	def __shift_arrow(self, arrow, dx, dy):
		new_points = []
		for x, y in arrow:
