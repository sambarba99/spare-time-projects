...
class DDQNAgent:
	...
	def do_training(self, env):
		...
		for i in range(1, N_EPOCHS + 1):
			...
			model_path = f'./ddqn/models/model_{laps:.2f}_laps_{mean_vel:.1f}_mean_vel.pth'
	...
	def choose_action(self, state):
		if random.random() < self.epsilon:
			...
			# Exploitation
			state = torch.tensor(state, dtype=torch.float32).unsqueeze(dim=0).cpu()
	...
	def experience_replay(self, batch, weights):
		...
		states = torch.tensor([s[0] for s in batch], dtype=torch.float32).cpu()
		actions = torch.tensor([s[1] for s in batch], dtype=torch.int64).cpu()
		returns = torch.tensor([s[2] for s in batch], dtype=torch.float32).cpu()
		next_states = torch.tensor([s[3] for s in batch], dtype=torch.float32).cpu()
		terminal_mask = torch.tensor([s[4] for s in batch], dtype=torch.bool).cpu()
	...
	def load_model(self):
		...
		self.policy_net = torch.load('./ddqn/models/model.pth')
