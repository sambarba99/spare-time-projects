...
class RolloutBuffer:
	...
	def rollout(self):
		batch_states = torch.stack(self.states).detach().cpu()
		batch_state_values = torch.stack(self.state_values).detach().cpu()
		batch_actions = torch.stack(self.actions).detach().cpu()
		batch_action_log_probs = torch.stack(self.action_log_probs).detach().cpu()
...
class PPOAgent:
	def __init__(self, *, training_mode):
		...
		self.trainable_policy = ActorCritic(self.training_mode).cpu()
		self.policy = ActorCritic(self.training_mode).cpu()
	...
	def do_training(self, env):
		...
		while timesteps_done < TOTAL_TRAIN_TIMESTEPS:                                                   # ALGORITHM STEP 2
			...
			model_path = f'./ppo/models/model_{laps:.2f}_laps_{mean_vel:.1f}_mean_vel.pth'
	...
	def compute_returns_to_go(self):
		...
		returns_to_go = torch.tensor(returns_to_go, dtype=torch.float32).cpu()
