	...
	if maze_mode:
		...
		for node in graph:
			for neighbour in node.neighbours:
				pg.draw.line(scene, (160, 160, 160), (node.j, node.i), (neighbour.j, neighbour.i))

		if path:
			for node, next_node in zip(path[:-1], path[1:]):
				pg.draw.line(scene, (0, 255, 0), (node.j, node.i), (next_node.j, next_node.i), 4)

		for node in graph:
			pg.draw.circle(scene, (255, 0, 0), (node.j, node.i), 3)

		# Start and target
		pg.draw.circle(scene, (0, 80, 255), (start_node.j, start_node.i), 6)
		pg.draw.circle(scene, (0, 80, 255), (target_node.j, target_node.i), 6)
