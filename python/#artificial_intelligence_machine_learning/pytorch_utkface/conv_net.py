...
class CNN(nn.Module):
	def __init__(self):
		...
		self.age_branch = nn.Linear(2048, 1)
		self.gender_branch = nn.Sequential(
			nn.Linear(2048, 1),
			nn.Sigmoid()
		)
		self.race_branch = nn.Sequential(
			nn.Linear(2048, 5),  # 5 race categories in dataset
			nn.Softmax(dim=-1)
		)

	def forward(self, x):
		conv_out = self.conv_block(x)
		# print(conv_out.shape)  # To get in_features of self.fc (e.g. [N, 64, 17, 14] -> 64x17x14 = 15232)
