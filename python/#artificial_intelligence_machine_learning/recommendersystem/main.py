...
class MatrixFactorisationModel(nn.Module):
	def __init__(self, n_users, n_shows, embedding_dim):
		super(MatrixFactorisationModel, self).__init__()
...
if __name__ == '__main__':
	...
	df = pd.read_csv(r'C:\Users\Sam\Desktop\Projects\datasets\show_ratings.csv')
	...
	train_show_ids = train_df['show_id'].values
	...
	x_train = torch.tensor(train_df[['user_id', 'show_id']].values, dtype=torch.long)
	y_train = torch.tensor(train_df['rating'].values, dtype=torch.float32)
	x_val = torch.tensor(val_df[['user_id', 'show_id']].values, dtype=torch.long)
	y_val = torch.tensor(val_df['rating'].values, dtype=torch.float32)

	# 3. Train embeddings

	model = MatrixFactorisationModel(n_users, n_shows, K).cpu()
	mae_loss = nn.L1Loss()
	...
	print(f'Train MAE: {train_loss.item()}')
	print(f'\n{train_df}')
	...
	x_test = torch.tensor(test_df[['user_id', 'show_id']].values, dtype=torch.long)
