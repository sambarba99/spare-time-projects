...
class MainPage(tk.Tk):
	...
	def update_table_with_filtered_results(self, *_):
	...
	def __sort_column(self, col, descending=False):
		...
		if col in ('ID', 'Year'):
			...
			# Sort by title, then by tkinter row ID
			l.sort(key=lambda item: (
				item[0].lower().removeprefix('the '),
				item[1]),
				reverse=descending)
		else:
