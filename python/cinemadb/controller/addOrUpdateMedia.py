"""
Cinema DB app add/update media page
...
class AddOrUpdateMediaPage(tk.Toplevel):
	...
	def __add_update_media(self):
		...
		for attr, val in self.media.__dict__.items():
			if not val and attr != 'mid':
				messagebox.showerror(title='Error', message=f"Media is missing attribute '{attr.replace('_', ' ')}'")
				return
			if attr == 'year' and re.match('^\\d{4}$', val) is None:
				messagebox.showerror(title='Error', message='Year must be a 4-digit number')
				return
	...
	def __update_media_and_text_area(self, *_):
