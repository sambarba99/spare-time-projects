...
<p align="justify">
	Instead of extracting overlapping tiles from a source image, this approach uses an input set of tiles with predefined adjacency rules (e.g. a list of valid left-right neighbour tile pairs, including rotations to infer up-down pairs too). When generating an output, the algorithm places tiles so that any adjacent pair obey these constraints. As usual, the adjacency rules are then propagated throughout the output grid via WFC to reduce the cells' tile options, and this process continues until all cells are left with one tile.
</p>

Example of generated output from `circuit` tiles and their `_left_right_adjacencies.json`:
...
<p align="center">
	<img src="adjacent/readme_imgs/water_tiles.png"/>
	<br/>
