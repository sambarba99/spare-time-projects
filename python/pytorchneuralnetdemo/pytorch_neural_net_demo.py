...
def confusion_matrix(predictions, actual):
	n_classes = len(actual.unique())
	conf_mat = np.zeros((n_classes, n_classes)).astype(int)

	for a, p in zip(actual, predictions):
		conf_mat[int(a.item())][int(p.item())] += 1

	return conf_mat

def plot_confusion_matrix(conf_mat, f1_score):
	ax = plt.subplot()
...
def main():
	...
	with torch.inference_mode():
		if task_choice == 'B':
			...
			print('Test F1 score:', test_metric)

			test_conf_mat = confusion_matrix(y_test_pred, y_test)
			plot_confusion_matrix(test_conf_mat, test_metric)
			...
			test_loss = loss_func(y_test_probs, y_test)
			test_metric = f1_score(y_test.argmax(dim=1), y_test_pred, average='weighted')

			print('Test loss:', test_loss.item())
			print('Test F1 score:', test_metric)

			test_conf_mat = confusion_matrix(y_test_pred, y_test.argmax(dim=1))
			plot_confusion_matrix(test_conf_mat, test_metric)
			...
			test_metric = mae_loss(y_test_pred, y_test)
